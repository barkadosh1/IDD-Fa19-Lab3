...
For my setup, I had both a potentiometer and FSR set up. I was using the potentiometer to read in and determine what state to follow and used the FSR to have its values measured in the write state. For the most part then, I only really changed code in the write state (except for changing code in SwitchState2 to add in the potentiometer and FSR). Within my write code, I am reading in the analog value every time the write state is called (through the SwitchState loop) and map it from the 0-1023 value range to the 0-255 value range. I start by writing this value to the 0th memory address and increment the memory address with each call to the write state. This writes a series of analog voltage values to the memory address and they are then printed during the read state. Once I enter the clear state, these values are removed and the memory addresses are filled with 0's. As I turn my potentiometer, it goes from clear --> read -- write. As I hold the FSR in the write state, it records analog voltage values. When I turn back into the read state, those values are printed and are then cleared when I return to the clear state.
...
The basic premise of my idea is the following. To align ones sleep cycle with the sunrise, I will have a photo cell sensor that measures to states -- one where the amount of sun is deemed too low and one where it has crossed the threshold (meaning the sun has risen). Once the photo cell detects that the sunrise has occured, an alarm (in this case flashing lights) will appear, prompting the user to turn it off through an LCD. To turn it off, the user will have to press on an FSR for 10 seconds with enough force. The FSR will have 3 possible states within the photo cell state where the sunrise has occured. The first if the user is not pressing the button and the LCD is telling them to press the FSR. The second is if the user is not pressing the FSR hard enough and the LCD prompts them to press harder. The last is when the user is holding the FSR hard enough -- in this case, the user must hold the FSR for 10 seconds. The LCD will then display how much time the user snoozed. 

<img src="https://github.com/barkadosh1/IDD-Fa19-Lab3/blob/master/Screen%20Shot%202019-09-27%20at%2011.37.45%20AM.png">

### 3. Create your data logger!
 
**a. Record and upload a short demo video of your logger in action.**

The basic idea is stated above and most of the technical aspects are stated as well. To reiterate, there are two "light" states -- one where not enough light/sun is present, and the other when enough sun/light is present. Once enough sun is present, the second state will be triggered and within that state, there are 3 FSR states: not pressing the FSR, pressing the FSR but not hard enough, and pressing the FSR hard enough (and the user will be required to press it for 10 seconds). Within my code, I wrote a function to track the time once the alarm is triggered (once enough sun appears). With this part of the lab, I wanted to better understand the put and get functions of EEPROM, so I used the put function to store my time value of type long in the EEPROM. The other high-level aspect of my code is that once the user successfully holds the FSR for 10 seconds (they can do many other things in between such as not press the FSR for some time, not press it hard for some time, press it hard enough for 5 seconds and then take their hand off and retry, etc.), the final time value is calculated and placed in the EEPROM. I then use the get method to access the start and stop times and calculate the amount of "snooze" time from when the alarm started to when the user successfully turned it off. This snooze time is printed to the LCD.
