...
The basic premise of my idea is the following: To align one's sleep cycle with the sunrise, I will have a photo cell sensor that measures two states -- one where the amount of sun is deemed too low and one where it has crossed the threshold (meaning the sun has risen). Once the photo cell detects that the sunrise has occured, an alarm (in this case flashing lights) will appear, prompting the user to turn it off through an FSR. To turn it off, the user will have to press on an FSR for 10 seconds with enough force. The FSR will have 3 possible states within the photo cell state where the sunrise has occured. The first is when the user is not pressing the button and the LCD is telling them to press the FSR. The second is when the user is not pressing the FSR hard enough and the LCD prompts them to press harder. The last is when the user is holding the FSR hard enough -- in this case, the user must hold the FSR for 10 seconds. The LCD will then display how much time the user spent snoozing. 
